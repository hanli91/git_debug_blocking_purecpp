...
void original_topk_plain_first_impl(const Table& ltoken_vector, const Table& rtoken_vector,
                                    Heap& topk_heap, PrefixHeap& prefix_events, Signal& signal, CandSet& cand_set,
    ...
    while (prefix_events.size() > 0) {
        ...
        if (signal.value != -1 && signal.value != 0) {
...
Heap original_topk_sim_join_plain_first(const Table& ltoken_vector, const Table& rtoken_vector, Signal& signal,
    ...
    Heap topk_heap;
    original_topk_plain_first_impl(ltoken_vector, rtoken_vector, topk_heap, prefix_events, signal,
